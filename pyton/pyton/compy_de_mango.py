...
def obtener_frase(modo, nivel, tipo):
    # Creamos una llave única para esta lista en la memoria (ej: mazo_peda_n1_v)
    llave_mazo = f"mazo_{modo}_{nivel}_{tipo}"
    ...
    if llave_mazo not in st.session_state or len(st.session_state[llave_mazo]) == 0:
        ...
        try:
            with open(ruta, "r", encoding="utf-8") as f:
                ...
                random.shuffle(preguntas) # Mezclamos para que el orden sea aleatorio
                st.session_state[llave_mazo] = preguntas
        except FileNotFoundError:
            return "No se encontró el archivo de preguntas."

    # Sacamos la primera pregunta y la eliminamos de la lista para que no se repita
    return st.session_state[llave_mazo].pop(0) 
# --- 4. LOBBY ---
